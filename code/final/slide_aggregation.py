...
log_path = os.path.join(model_save_path, 'slide_predictions', 'slide_predictions_log.txt')
...
def majority_voting(test_generator, y_mc):
    ...
    for i in range(ntest):
        ...
        if i == ntest-1:
            slide_predictions.append(np.mean(patch_predictions))
    return np.round(slide_predictions)
...
def logistic_regression_aggregation(test_generator, valid_generator, y_mc, y_mc_valid):
    ...
    def build_df(generator, y_mc):
        ...
        for i in range(n):
            ...
            if slide == current_slide:
                ...
                if y_pred[i] == 0 and weights[i]>=0.95:
                    GBM95_count += 1
                if y_pred[i] == 0 and weights[i]>=0.9 and weights[i]<0.95:
                    GBM90_count += 1
                if y_pred[i] == 0 and weights[i]>=0.85 and weights[i]<0.9:
                    GBM85_count += 1
                if y_pred[i] == 0 and weights[i]>=0.8 and weights[i]<0.85:
                    GBM80_count += 1
                if y_pred[i] == 0 and weights[i]>=0.75 and weights[i]<0.8:
                    GBM75_count += 1
                if y_pred[i] == 0 and weights[i]>=0.7 and weights[i]<0.75:
                    GBM70_count += 1
                if y_pred[i] == 0 and weights[i]>=0.65 and weights[i]<0.7:
                    GBM65_count += 1
                if y_pred[i] == 0 and weights[i]>=0.6 and weights[i]<0.65:
                    GBM60_count += 1
                if y_pred[i] == 0 and weights[i]>=0.55 and weights[i]<0.6:
                    GBM55_count += 1
                if y_pred[i] == 0 and weights[i]>=0.5 and weights[i]<0.55:
                    GBM50_count += 1
                if y_pred[i] == 0 and weights[i]<0.5:
                    GBM0_count += 1
                if y_pred[i] == 1 and weights[i]>=0.95:
                    LGG95_count += 1
                if y_pred[i] == 1 and weights[i]>=0.9 and weights[i]<0.95:
                    LGG90_count += 1
                if y_pred[i] == 1 and weights[i]>=0.85 and weights[i]<0.9:
                    LGG85_count += 1
                if y_pred[i] == 1 and weights[i]>=0.8 and weights[i]<0.85:
                    LGG80_count += 1
                if y_pred[i] == 1 and weights[i]>=0.75 and weights[i]<0.8:
                    LGG75_count += 1
                if y_pred[i] == 1 and weights[i]>=0.7 and weights[i]<0.75:
                    LGG70_count += 1
                if y_pred[i] == 1 and weights[i]>=0.65 and weights[i]<0.7:
                    LGG65_count += 1
                if y_pred[i] == 1 and weights[i]>=0.6 and weights[i]<0.65:
                    LGG60_count += 1
                if y_pred[i] == 1 and weights[i]>=0.55 and weights[i]<0.6:
                    LGG55_count += 1
                if y_pred[i] == 1 and weights[i]>=0.5 and weights[i]<0.55:
                    LGG50_count += 1
                if y_pred[i] == 1 and weights[i]<0.5:
                    LGG0_count += 1
                patch_count += 1
            else:
                # append one count to list
                GBM95.append(GBM95_count)
                GBM90.append(GBM90_count)
                GBM85.append(GBM85_count)
                GBM80.append(GBM80_count)
                GBM75.append(GBM75_count)
                GBM70.append(GBM70_count)
                GBM65.append(GBM65_count)
                GBM60.append(GBM60_count)
                GBM55.append(GBM55_count)
                GBM50.append(GBM50_count)
                GBM0.append(GBM0_count)
                LGG95.append(LGG95_count)
                LGG90.append(LGG90_count)
                LGG85.append(LGG85_count)
                LGG80.append(LGG80_count)
                LGG75.append(LGG75_count)
                LGG70.append(LGG70_count)
                LGG65.append(LGG65_count)
                LGG60.append(LGG60_count)
                LGG55.append(LGG55_count)
                LGG50.append(LGG50_count)
                LGG0.append(LGG0_count)
        ...
        logreg_df['GBM90'] = GBM90
        logreg_df['GBM85'] = GBM85
        logreg_df['GBM80'] = GBM80
        logreg_df['GBM75'] = GBM75
        logreg_df['GBM70'] = GBM70
        logreg_df['GBM65'] = GBM65
        logreg_df['GBM60'] = GBM60
        logreg_df['GBM55'] = GBM55
        logreg_df['GBM50'] = GBM50
        logreg_df['GBM0'] = GBM0
        logreg_df['LGG95'] = LGG95
        logreg_df['LGG90'] = LGG90
        logreg_df['LGG85'] = LGG85
        logreg_df['LGG80'] = LGG80
        logreg_df['LGG75'] = LGG75
        logreg_df['LGG70'] = LGG70
        logreg_df['LGG65'] = LGG65
        logreg_df['LGG60'] = LGG60
        logreg_df['LGG55'] = LGG55
        logreg_df['LGG50'] = LGG50
        logreg_df['LGG0'] = LGG0
        logreg_df['total_patches'] = patches
        logreg_df['slide_label'] = slide_labels

        # create ratios df
        logreg_df_ratios = pd.DataFrame({})
        logreg_df_ratios['GBM95_ratio'] = logreg_df.GBM95 / logreg_df.total_patches
        logreg_df_ratios['GBM90_ratio'] = logreg_df.GBM90 / logreg_df.total_patches
        logreg_df_ratios['GBM85_ratio'] = logreg_df.GBM85 / logreg_df.total_patches
        logreg_df_ratios['GBM80_ratio'] = logreg_df.GBM80 / logreg_df.total_patches
        logreg_df_ratios['GBM75_ratio'] = logreg_df.GBM75 / logreg_df.total_patches
        logreg_df_ratios['GBM70_ratio'] = logreg_df.GBM70 / logreg_df.total_patches
        logreg_df_ratios['GBM65_ratio'] = logreg_df.GBM65 / logreg_df.total_patches
        logreg_df_ratios['GBM60_ratio'] = logreg_df.GBM60 / logreg_df.total_patches
        logreg_df_ratios['GBM55_ratio'] = logreg_df.GBM55 / logreg_df.total_patches
        logreg_df_ratios['GBM50_ratio'] = logreg_df.GBM50 / logreg_df.total_patches
        logreg_df_ratios['GBM0_ratio'] = logreg_df.GBM0 / logreg_df.total_patches
        logreg_df_ratios['LGG95_ratio'] = logreg_df.LGG95 / logreg_df.total_patches
        logreg_df_ratios['LGG90_ratio'] = logreg_df.LGG90 / logreg_df.total_patches
        logreg_df_ratios['LGG85_ratio'] = logreg_df.LGG85 / logreg_df.total_patches
        logreg_df_ratios['LGG80_ratio'] = logreg_df.LGG80 / logreg_df.total_patches
        logreg_df_ratios['LGG75_ratio'] = logreg_df.LGG75 / logreg_df.total_patches
        logreg_df_ratios['LGG70_ratio'] = logreg_df.LGG70 / logreg_df.total_patches
        logreg_df_ratios['LGG65_ratio'] = logreg_df.LGG65 / logreg_df.total_patches
        logreg_df_ratios['LGG60_ratio'] = logreg_df.LGG60 / logreg_df.total_patches
        logreg_df_ratios['LGG55_ratio'] = logreg_df.LGG55 / logreg_df.total_patches
        logreg_df_ratios['LGG50_ratio'] = logreg_df.LGG50 / logreg_df.total_patches
        logreg_df_ratios['LGG0_ratio'] = logreg_df.LGG0 / logreg_df.total_patches
        logreg_df_ratios['slide_label'] = slide_labels
    ...
    # logistic regression
    logreg_X_valid = df_valid.iloc[:,:-1]
    logreg_Y_valid = df_valid.iloc[:,-1]

    logreg_X_test = df_test.iloc[:,:-1]
    logreg_Y_test = df_test.iloc[:,-1]

    logreg = LogisticRegression(solver='lbfgs',random_state=0)
    logreg.fit(logreg_X_valid, logreg_Y_valid)

    slide_predictions = logreg.predict(logreg_X_test)
    return slide_predictions
...
def spatial_flipping(test_generator, y_mc, threshold):
    ...
    for slide in file_start_dict:
        ...
        slide_prediction = np.round(np.mean(y_pred_patches_updated))
...
print('Logistic regression predictions: ' + str(logistic_regression_aggregation_predictions))
logfile = open(log_path,'a')
logfile.write('\n')
logfile.write('\nLogistic regression predictions: ' + str(logistic_regression_aggregation_predictions))
logfile.close()
...
for threshold in thresholds:
    ...
    tn, fp, fn, tp = metrics.confusion_matrix(slide_labels, spatial_flipping_predictions).ravel()
    slide_accuracy = (tn + tp) / (tn + fp + fn + tp)
    slide_accuracy
    accs.append(slide_accuracy)
