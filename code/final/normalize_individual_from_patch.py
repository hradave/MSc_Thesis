...
# normalize wsi
start_norm = timer()
filtered_np[np.where((filtered_np==[0,0,0]).all(axis=2))] = [255,255,255]
filtered_np = filtered_np.copy()
filtered_np = normalizer.transform(filtered_np)
end_norm = timer()
print("Time to normalize wsi: %s\n" % str(end_norm-start_norm))

logfile = open(os.path.join(base_path, 'dev', 'test', 'preprocess_log.txt'),'a')
logfile.write('\nNormalizing done: ' + str(end_norm - start_norm))
logfile.close()

# save normalized image
start_save2 = timer()
filtered = util.np_to_pil(filtered_np)
filtered.save(os.path.join(base_path, 'dev', 'test', gbm_lgg + '_' + file_name[:-4] + '_normalized' + '.png'))
end_save2 = timer()
print("Time to save normalized image: %s\n" % str(end_save2 - start_save2))

logfile = open(os.path.join(base_path, 'dev', 'test', 'preprocess_log.txt'),'a')
logfile.write('\nSaving done: ' + str(end_save2 - start_save2))
logfile.close()
