    ...
    for r in range(patches.shape[0]):
        for c in range(patches.shape[1]):
            ...
            if filter.tissue_percent(patch) > 90:
                ...
                patch.save(os.path.join(tiles_path, file_name[:-52] + '__' + str(r) + '-' + str(c) + '_' + str(count_good_patches) + '.png'))
                count_good_patches += 1
    return [file_name[:-52], count_all_patches, count_good_patches]



# file paths
base_path = '/nobackup/data/davhr856/data/TCGA_DX/sample'
path = '/nobackup/data/davhr856/data/TCGA_DX/sample/filtered_after_normalize'
# result path
tiles_path = '/nobackup/data/davhr856/data/TCGA_DX/sample/tiles'
